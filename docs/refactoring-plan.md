...
- [x] `value` オブジェクトを `useMemo` 化（現状毎レンダー新規生成で、全消費者が毎回再レンダー）→ さらに Busy / Data / Actions の Context 分割
...
- [x] コメント（「原子的に置換」）と実装の不一致を解消: tmp ファイルに書いて `rename()` するアトミック置換に変更
- [x] GET 時に共有ロック（`flock LOCK_SH`）を取る（PUT の truncate 中に読むと壊れた JSON を返し得る）
- [x] `fwrite` の部分書き込み検出を追加
- [x] CORS の任意 Origin 反射は**本番導入時に絞る**（TODO として残す。今は開発都合で現状維持でよいか要確認）→ **現状維持** + `TODO(本番)` コメント明記
